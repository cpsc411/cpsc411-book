...
@margin-note{If we keep track of the arity of procedures, we can optimize this
...
blocks of code.
Unfortunately, now our procedures can contain references to @tech{free}
...
@ch6-tech{procedure} from @ch6-tech{Values-lang v6}.
...
We also add a new form for dereferencing the value of @ch3-tech{lexical
variables} from the @tech{closure} @closure-lang-v9[(closure-ref e e)].
...
@margin-note{
If the operator is already an @closure-lang-v9[aloc], avoid introducing an extra
@closure-lang-v9[let]:
@racketblock[
...
where @racket[label] is known to be the label of the closure @racket[value].
Because @racket[value] is already an @closure-lang-v9[aloc], we can safely
discard it; we do not need to force evaluation to preserve any side effects.
...
@racket[value] and @racket[lam] can be transformed into @closure-lang-v9[(call
label_c values ...)].

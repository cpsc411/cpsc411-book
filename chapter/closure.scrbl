...
This week, we'll add first-class procedures as values in @deftech{Exprs-lang v9}:
@bettergrammar*-ndiff[
...
Before we can generate labelled procedures, we collect the free-variable
information and create an explicit procedure data structure.
This data structure implements a @deftech{closure}, a procedure paired with the
values of free variables from the environment in which the procedure was created.

We do this in two steps.

@subsection{uncover-free}
First, we uncover the free variables in each @lam-free-lang-v9[lambda].
We add these as an annotation on the @lam-free-lang-v9[lambda], which the next pass
...
@todo{Define free and bound; probably in an intro section explaining all the
concepts}
To find the free @ch3-tech{abstract locations}, we traverse the body of each
@lam-free-lang-v9[lambda] remembering any @ch3-tech{abstract locations} that
have been bound (by @lam-free-lang-v9[let], @lam-free-lang-v9[lambda], or
...
@lam-free-lang-v9[(aloc ...)] are considered bound, initially.
...
Even a variable bound in a @lam-free-lang-v9[letrec] is considered free in the body
of a @lam-free-lang-v9[lambda].
@examples[#:eval sb
(uncover-free
 `(module
...
First, you can apply a @tech{closure} with @closure-lang-v9[closure-apply], which
...
@tech{closure} with @closure-lang-v9[closure-ref], extracting the value of a closure
at an index.
...
@item{@closure-lang-v9[(closure-apply e_c es ...)]

Safely apply the @tech{closure} @closure-lang-v9[e_c], invoking its @tech{code}, to
...
The abstract location to which the the @closure-lang-v9[lambda] was previously bound
...
To support the translation, we allow @closure-lang-v9[let] to bind multiple abstract
locations at once.
We add the number of arguments as a field in the closure to implement safe
application later.
}
@item{Transform each @closure-lang-v9[apply].
...
We use @closure-lang-v9[closure-apply] to apply the (label of the) closure to
the closure itself and its usual arguments.
We need to bind the operator to avoid duplicating code.

However, if the operator is already a @closure-lang-v9[aloc], we should instead
...
We add @closure-lang-v9[unsafe-apply] to the language to enable optimizing closures,
an important optimization in functional languages.
This @closure-lang-v9[unsafe-apply] directly applies a label to arguments, without
performing any checks.
@closure-lang-v9[closure-apply] will get translated into the safe, dynamically
checked apply.
}
]

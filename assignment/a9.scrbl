...
@(require cpsc411/deprecated/a9-compiler-lib)
...
@title[#:tag "top" #:tag-prefix "a9:"]{Compiler 9: First-class Functions}

@section{Assignment Summary}

The goal of this assignment is to add first-class functions to our language.
This extends the procedure data types to include the values of free variables at
the procedure's definition.
We add @object-code{lambda} as a first-class value in the source language, which
...
@url{https://www.students.cs.ubc.ca/~cs-411/2019w2/a9-interrogator.cgi}

@subsubsub*section{Assignment Checklist}
You should find a new repository in your
@url{https://github.students.cs.ubc.ca} account named @tt{a9_<team ids>} with a
...
You should first merge your solution to @secref[#:tag-prefixes '("a8:")]{top}
...
The name of the skeleton is @share{a9-skeleton.rkt} to avoid accidentally
...
@a8-tech{Exprs-lang v8} gave us the ability to implement structured,
heap-allocated data.
The language is now as expressive as many general-purpose programming languages.
You could start building large programs and standard libraries, with only a few
small modifications to the run-time system to enable dynamic input and output.

However, it is missing a useful feature for software development: the ability to
easily abstract over computations at any point via first-class functions.
